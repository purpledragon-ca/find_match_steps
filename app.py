...
session: dict = {
    ...
    'matches': None,       # optional matcher output JSON
}


def _load_matches(matches_path: Path | None) -> dict | None:
    """Load optional component match results generated by match_step_component_positions.py."""
    if not matches_path:
        return None
...
def preload_step(step_path: Path, matches_path: Path | None = None):
    ...
    if matches_path:
        count = len((session.get('matches') or {}).get('matches', []))
        print(f'Pre-loaded matches: {matches_path} ({count} instances)', file=sys.stderr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matches', metavar='JSON', help='Pre-load match JSON and highlight matches')
    args = parser.parse_args()

    if args.step:
        preload_step(Path(args.step), Path(args.matches) if args.matches else None)
